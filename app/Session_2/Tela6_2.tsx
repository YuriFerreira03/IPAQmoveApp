...
type TelaLocalizacaoRouteProp = RouteProp<RootStackParamList, "Tela6_2">;

const Tela6_2: React.FC<{ route: TelaLocalizacaoRouteProp }> = ({ route }) => {
  ...
  const fetchQuestao = async () => {
    try {
      ...
      const url = `http://${ip}:8080/questao/12`; // Passando o id_questao diretamente so colocar o numero de acordo com o banco
  ...
  const handleRegister = async () => {
    ...
    try {
      if (isChecked) {
        console.log("Checkbox marcado, navegando para Seção3");
        navigation.navigate("Splash3"); // Navega para Tela3_2 se o checkbox estiver marcado
      ...
      console.log("Enviando dados para o backend:", {
        ...
        fk_Questionario_id_questao: 12, // Substitua pelo ID da questão correta
      ...
      const dadosParaEnvio = {
        fk_Usuario_id_usuario: userId, // Utilize o ID do usuário logado
        fk_Questionario_id_questao: 12, // Sempre define com o id da questao
      ...
      console.log("Navegando para Tela7_2");
      setSearchName("");
      navigation.navigate("Tela7_2");
    } catch (error) {
